...
{	
	...
	if(stackIsEmpty(s) == true)
		...
		cout << "Ngan xep co phan tu" << endl;
	
	int x;
	int n;
	int a;
	cout << "Nhap so phan tu can chen: n = ";
	cin >> n;
	for(int i = 0 ; i < n ; i++)
	{
		cout << "Nhap phan tu vao ngan: ";
		cin >> a;
		stackPush(s , a);
	}
...
void check(Stack & stack , int x)
{
	...
	if(count = 0) cout << "Khong co phan tu trong ngan xep" << endl;
